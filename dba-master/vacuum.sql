select table_catalog, table_schema, table_name from information_schema.tables where table_schema='public' and table_type='BASE TABLE' order by table_name;
select 'VACUUM FULL VERBOSE ANALYZE '|| table_name ||';' from information_schema.tables where table_schema='public' and table_type='BASE TABLE' order by table_name;
--For DAISEY use table_schema='kehs'
...
left JOIN pg_class AS tbl ON att.attrelid = tbl.oid
left JOIN pg_namespace AS ns ON ns.oid = tbl.relnamespace
...
ORDER BY tblname--2,3
) AS s
) AS s2
) AS s3
where schemaname='public';

-- select (159825920::numeric/ 181747712) 
