    ...
    cursor1.execute("""
      ...
      WITH nonqc_schools as ( select schoolid  from organizationtreedetail where stateid not in (select id from organization where  organizationtypeid=2 and 
		...
		e.currentschoolyear, s.firstlanguage,
				...
				sap.activeflag activeassessmentflag
            FROM student s
    ...
    for row in cursor1:
        ...
        sqlitecur.execute("""
            INSERT INTO student (id, activeflag, enrollmentid, legalfirstname,
                ...
                activeenrollmentflag, exitwithdrawaldate, exitwithdrawaltype,sci_band,final_sci,writing_band,activeassessmentflag
                )
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        """,
                          [
...
def extract_student_tests(options, db, sqliteconn):
    ...
    if options.assessment == '12' or options.assessment == '37' or options.assessment == '47':
		...
		specialcircumstancecedscode integer,
				...
				rosteridtested integer,
				teacheridtested integer
            );
            """)
 ...
 ,'Flatland','KAP QC State','MY ORGANIZATION ','NY Training State','PII Deactivation','Playground QC State','QA QC State','Service Desk QC State','Training Program')))					
                ...
                INSERT INTO studentstests (id, studentid, enrollmentid, testid,
                        ...
                        educatorid, finalband, programname, paperpencil,rosterid,contentareaname,interimtheta,osbrowser,specialcircumstanceid,
		                      specialcircumstancetype,
                                    ...
                                    specialcircumstanceactive, totalrawscore,rosteridtested,teacheridtested )
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""", row)
        ...
        cursor1.execute("""
            ...
            when ts.source='MANUAL' and tc.name ~* 'Offline' then 'Year end'
				...
				st.rosteridtested ,
				st.teacheridtested, 
				str.coursesectionname rostertested
            FROM studentstests st
            JOIN test t ON st.testid = t.id
            JOIN studentstestsections sts ON sts.studentstestid = st.id
            JOIN testcollectionstests tct ON st.testid = tct.testid
            ...
            LEFT JOIN category fband ON st.finalbandid = fband.id
            LEFT JOIN roster r ON ts.rosterid = r.id
			LEFT JOIN roster str ON st.rosteridtested = str.id
    ...
    if options.assessment == '3':
        ...
        for row in cursor1:
            ...
            sqlitecur.execute("""
				...
				source,
                testcollectionid,
				model,rosteridtested ,
				teacheridtested,rostertested )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?,?,?,?)""",
                          row)
            ...
            sqlitecur.execute("""
                INSERT INTO studentstests_interim (id, studentid, enrollmentid, testid,
                        ...
                        educatorid, finalband, programname, paperpencil,rosterid,contentareaname,interimtheta,osbrowser,specialcircumstanceid,
		                      specialcircumstancetype,
		                            specialcircumstancecedscode,
                                    specialcircumstanceactive, totalrawscore,testspecification, rosteridtested ,
				teacheridtested )
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                        ...
                        row['specialcircumstanceactive'],row['totalrawscore'],row['testspecification'],row['rosteridtested'],row['teacheridtested']))
