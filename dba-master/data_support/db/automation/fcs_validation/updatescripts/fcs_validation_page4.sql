--1.a:validation: Find the all mandatory questions were not answered and iscompleted set true (note:radio button)
...
--1.b:validation: Find the all mandatory questions were answered and iscompleted set false (note:radio button)
update surveypagestatus
set iscompleted =false,
    modifieddate=now(),
    modifieduser=12
...
with all_ques as
(
select sv.id surveyid,sl.globalpagenum,count(distinct case when  sl.id in (81,82,83,84) then 81 else sl.id end ) cnt
from survey sv
inner join student s                       on s.id=sv.studentid and s.activeflag is true and sv.activeflag is true
inner JOIN enrollment e                    ON e.studentid = s.id and e.activeflag is true
inner join organization o                  on s.stateid=o.id and o.activeflag is true
...
where e.currentschoolyear=2017  and sl.activeflag is true 
and optional= case when cafcss.categorycode='CORE_SET_QUESTIONS' then false else optional end
and sl.globalpagenum=4 and sl.id in (81,82,83,84,137) --(Q29_1,Q29_2,Q29_3,Q29_4,Q200)
group by sv.id,sl.globalpagenum
)
,all_answ as
(
select sv.id surveyid,sp.globalpagenum,count(distinct case when  sl.id in (81,82,83,84) then 81 else sl.id end ) cnt
from survey sv
inner join student s                       on s.id=sv.studentid and s.activeflag is true and sv.activeflag is true
inner JOIN enrollment e                    ON e.studentid = s.id and e.activeflag is true
inner join organization o                  on s.stateid=o.id and o.activeflag is true
...
inner join surveypagestatus sp on sp.surveyid=tgt.surveyid and tgt.globalpagenum=sp.globalpagenum and sp.activeflag is true
left outer join all_answ src on tgt.surveyid=src.surveyid and tgt.globalpagenum=src.globalpagenum
where coalesce(src.cnt,0)<2 and sp.iscompleted is true);

--1.c:validation: Find the all mandatory questions were not answered and iscompleted set true 
update surveypagestatus
