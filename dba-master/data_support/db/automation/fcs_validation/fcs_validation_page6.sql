...
and sl.globalpagenum=6 and sl.id=3 --(Q36)
...
--1.c:validation: Find the all mandatory questions were not answered and iscompleted set true (note:Q36 response Yes)
with all_ques as
(
select sv.id surveyid,sp.globalpagenum,count(distinct sl.labelnumber) cnt
from survey sv
...
where e.currentschoolyear=2017  and sl.activeflag is true 
and optional= case when cafcss.categorycode='CORE_SET_QUESTIONS' then false else sl.optional end
and sl.globalpagenum=6 and sl.id=4 --(Q37)
group by sv.id,sp.globalpagenum
)
select tgt.surveyid,tgt.globalpagenum,sp.id pageid,tgt.cnt questionscount, src.cnt answerscount
from all_ques tgt
...
--1.d:validation: Find the all mandatory questions were answered and iscompleted set false  (note:Q36 response Yes) 
with all_ques as
(
select sv.id surveyid,sp.globalpagenum,count(distinct sl.labelnumber) cnt
from survey sv
