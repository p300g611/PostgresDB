...
FOR tmp_table IN (SELECT trim(statestudent_identifier)   statestudent_identifier,
 ...
 LOOP   
	...
	RAISE NOTICE 'processing row(%)>> Student_State_ID:% , ayp_sch_displayidentifier: %, att_sch_displayidentifier: %, exit_att_sch_displayidentifier: %'
        ...
        BEGIN
         FOR tmp_enrollmentid in (select e.id,e.studentid
					...
					and lower(o.displayidentifier) = lower(tmp_table.exit_att_sch_displayidentifier)
					and tmp_table.exit_att_sch_displayidentifier IS NOT NULL
			           )		
                       LOOP 
					 update enrollment
						set activeflag = false,
						exitwithdrawaldate = now_date,
						exitwithdrawaltype = -55,
						modifieddate = now_date,
						modifieduser = (select id from aartuser where username = 'cetesysadmin'),
						notes = 'Deactivated enrollment on '||now_date::text
					 where  id = tmp_enrollmentid.id
						and studentid=tmp_enrollmentid.studentid;	
			  RAISE NOTICE 'Deactivated enrollmentid:% , for studentid:% ',tmp_enrollmentid.id,tmp_enrollmentid.studentid;
