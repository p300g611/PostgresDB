...
psql -h aartdb-stage.c4d5gwuuaeq5.us-east-1.rds.amazonaws.com -U aart_user -f "$scripts/upload_moodle.sql" -d aartstage &> $scripts/upload_moodle.out
...
sftp -o IdentityFile=~/.ssh/ku-upload.key ku-upload@ku.rms.webanywhere.co.uk <<EOF
  mput $export/upload_moodle_$todaydate.csv /staging/incoming/
  bye
EOF
