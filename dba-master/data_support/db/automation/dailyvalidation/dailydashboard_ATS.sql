...
sum(nooftests) nooftests,sum(nooftests_lastday) nooftests_lastday,sum(nooftests_today) nooftests_today,sum(nostage2testscompleted) nostage2testscompleted into temp dailydashboard_newsessions
...
\copy (select * from dashboardreactivationsdaily) to '/srv/extracts/helpdesk/dailyvalidation/data/dashboardreactivationsdaily.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);
...
\copy (select * from dashboardtestingoutsidehoursdaily) to '/srv/extracts/helpdesk/dailyvalidation/data/dashboardtestingoutsidehoursdaily.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);
...
\copy (select * from dashboardstudentscompleteddaily) to '/srv/extracts/helpdesk/dailyvalidation/data/dashboardstudentscompleteddaily.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);
...
select  statename,districtname,districtdbid,schoolname,schooldbid,grade,gradebandname,contentarea,
 roster,rosterdbid,teacher,teacherdbid,operationaltestwindowid,testcollectionname,status,nooftests
...
\copy (select * from dashboardstudentsassigneddaily) to '/srv/extracts/helpdesk/dailyvalidation/data/dashboardstudentsassigneddaily.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);
*/
...
inner join gradecourse gc on tc.gradecourseid=gc.id and gc.activeflag is true 
where ts.activeflag is true and ts.schoolyear=2017 and st.activeflag is true 
and ts.source='BATCHAUTO' and ts.operationaltestwindowid =10161;
