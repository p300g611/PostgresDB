...
\COPY tmp_kap_dlm_corrections_raw FROM '/CETE_GENERAL/Technology/helpdesk/datasupport/scripts/unprocessed/kap_dlm_corrections.csv' DELIMITER ',' CSV HEADER ;  
   ...
   FOR data IN (SELECT row_num
		...
		IF currentstudentid IS NOT NULL THEN
			...
			IF (data.action = 'A') THEN 
			   PERFORM reactivate_student_from_assessment_program(currentstudentid, data.programabbreviation, 2016);
...
END;
$BODY$;
\COPY (select * from tmp_kap_dlm_corrections where error_msg ilike '%<error>%' ) to '/CETE_GENERAL/Technology/helpdesk/datasupport/kap_dlm_corrections_error.csv' DELIMITER ',' CSV HEADER ;
drop table if exists  tmp_kap_dlm_corrections ;
drop table if exists  tmp_kap_dlm_corrections_raw ;    
