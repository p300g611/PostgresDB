--File upgraded from the CB_ItemKey2016 file (This is for yearly item validation we added content code that duplicates the items keys)
...
-- Note: media--contextmediavariantid=mediavariantid (select count(distinct taskvariantid) from cb.taskvariantmediavariant)
...
/*
--yearly validation   
-- 6.List all items in Content Builder without item statistics 
-- 7 List all items in Content Builder in an accommodative test form without TTS tagged
-- 8 List all items in Content Builder with a scoring value set to null
 alter table tmp_final_report add  item_statistics_count int;
 alter table tmp_final_report add  braille_count int;
 alter table tmp_final_report add  signed_count int;
 alter table tmp_final_report add  readaloud_count int;
 alter table tmp_final_report add  testingprogram text;
 alter table tmp_final_report add  accessibilityflag text;




update tmp_final_report src
set item_statistics_count =tgt.cnt
from (select taskvariantid,organizationid,count(*) cnt from  cb.itemstatistic it 
      where taskvariantid is not null group by taskvariantid,organizationid) tgt
where src.externaltaskvariantid=tgt.taskvariantid and src.organizationid=tgt.organizationid;


update tmp_final_report src
set braille_count =tgt.cnt
from (select testdevelopmentid,organizationid,count(*) cnt from cb.brailleaccommodation ba 
  inner join cb.accessibilityfile a on a.accessibilityfileid=ba.accessibilityfileid
  where testdevelopmentid is not null
  group by testdevelopmentid,organizationid) tgt
where src.testdevelopmentid=tgt.testdevelopmentid and src.organizationid=tgt.organizationid;


update tmp_final_report src
set signed_count =tgt.cnt
from (select taskvariantid,organizationid,count(*) cnt from cb.signedaccommodation ba 
inner join cb.accessibilityfile a on a.accessibilityfileid=ba.accessibilityfileid
where taskvariantid is not null
group by taskvariantid,organizationid) tgt
where src.externaltaskvariantid=tgt.taskvariantid and src.organizationid=tgt.organizationid;

update tmp_final_report src
set readaloud_count =tgt.cnt
from (select taskvariantid,organizationid,count(*) cnt  from cb.readaloudaccommodation ba 
inner join cb.accessibilityfile a on a.accessibilityfileid=ba.accessibilityfileid
where taskvariantid is not null
group by taskvariantid,organizationid) tgt
where src.externaltaskvariantid=tgt.taskvariantid and src.organizationid=tgt.organizationid;
...
update tmp_final_report src
set testingprogram =tgt.name
from (select name,t.taskid,t.organizationid from cb.task t
join cb.testingprogram tp on tp.testingprogramid=t.testingprogramid and t.organizationid=tp.organizationid ) tgt
where src.externaltaskid=tgt.taskid and src.organizationid=tgt.organizationid;


update tmp_final_report src
set accessibilityflag =tgt.accessibilityflagcode
from (select testdevelopmentid, array_to_string(array_agg(distinct accessibilityflagcode::text), ',')  accessibilityflagcode
from  cb.testdevelopmentaccessibilityflag
group by testdevelopmentid) tgt
where src.testdevelopmentid=tgt.testdevelopmentid;
\copy (select distinct * from tmp_final_report ) to 'tmp_final_report_tests.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);   

select *, row_number() over (partition by externaltaskvariantid order by testdevelopmentid desc) row_num 
    into temp tmp_final_report_item
 from tmp_final_report; 

\copy (select * from tmp_final_report_item where row_num=1) to 'tmp_final_report_item.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *); 


*/

--media validation for items 
...
from cb.taskvariant tv 
   ...
   left outer join cb.systemrecord stst on stst.id=tst.systemrecordid  and stst.inuse is true 
   left outer join cb.cognitivetaxonomydimension ctd on ctd.cognitivetaxonomydimensionid=t.cognitivetaxonomydimensionid  and ctd.inuse is true and o.organizationid=ctd.organizationid
   left outer join (select tv.taskvariantid,m.mediaid from cb.taskvariant tv
	inner join cb.taskvariantmediavariant tvm on tv.taskvariantid=tvm.taskvariantid
	inner join cb.mediavariant m on m.mediavariantid=tvm.mediavariantid and m.inuse is true 
	union
	select tv.taskvariantid,m.mediaid from cb.taskvariant tv
	inner join cb.mediavariant m on m.mediavariantid=tv.contextmediavariantid and m.inuse is true) tvm on tv.taskvariantid=tvm.taskvariantid
   left outer join cb.systemrecord gb on t.gradebandid=gb.id      
   where  tv.inuse is true and t.inuse is true
         and o.organizationid  =15009   --only kap
 ...
 alter table tmp_media_report add  item_statistics_count int;
 alter table tmp_media_report add  braille_count int;
 alter table tmp_media_report add  signed_count int;
 alter table tmp_media_report add  readaloud_count int;
 alter table tmp_media_report add  testingprogram text;
 alter table tmp_media_report add  accessibilityflag text;
 alter table tmp_media_report add  taskvariant_audiotag text;
 alter table tmp_media_report add  taskvariantmedia_audiotag text;



update tmp_media_report src
set item_statistics_count =tgt.cnt
from (select taskvariantid,organizationid,count(*) cnt from  cb.itemstatistic it 
      where taskvariantid is not null group by taskvariantid,organizationid) tgt
where src.externaltaskvariantid=tgt.taskvariantid and src.organizationid=tgt.organizationid;
...
update tmp_media_report src
set taskvariant_audiotag ='<audioOR<video'
from (select taskvariantid,organizationid from cb.taskvariant 
where taskstem ilike '%<audio%' or taskstem ilike '%<video%') tgt
where src.externaltaskvariantid=tgt.taskvariantid and src.organizationid=tgt.organizationid;


update tmp_media_report src
set taskvariantmedia_audiotag ='<audioOR<video'
from (select tv.taskvariantid from cb.taskvariant tv
inner join cb.taskvariantmediavariant tvm on tv.taskvariantid=tvm.taskvariantid
inner join cb.mediavariant m on m.mediavariantid=tvm.mediavariantid 
where mediavariantcontent ilike '%<audio%' or mediavariantcontent ilike '%<video%'
union
select tv.taskvariantid from cb.taskvariant tv
inner join cb.mediavariant m on m.mediavariantid=tv.contextmediavariantid
where mediavariantcontent ilike '%<audio%' or mediavariantcontent ilike '%<video%') tgt
where src.externaltaskvariantid=tgt.taskvariantid;


\copy (select * from tmp_media_report) to 'tmp_media_report.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *); 
