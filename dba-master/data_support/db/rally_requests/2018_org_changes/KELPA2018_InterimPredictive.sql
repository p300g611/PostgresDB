...
select      distinct 
            ...
            e.currentgradelevel currentgradelevel, 
...
where ot.statedisplayidentifier='KS' and ap.abbreviatedname='K-ELPA' and e.currentschoolyear=2017
      and sap.activeflag is false  and gc.abbreviatedname in ('K','1','2')
...
inner join gradecourse gc on e.currentgradelevel=gc.id
where ot.statedisplayidentifier='KS' and ap.abbreviatedname='K-ELPA' and e.currentschoolyear=2017
      and kelpa2017flag=true and sap.activeflag is false  and gc.abbreviatedname in ('K','1','2')
      and not exists (select 1 from enrollment enrl where enrl.studentid=e.studentid and enrl.currentschoolyear=2018 and enrl.notes not like '%#$2018$#%');
...
inner join gradecourse gc on e.currentgradelevel=gc.id
where ot.statedisplayidentifier='KS' and ap.abbreviatedname='K-ELPA' and e.currentschoolyear=2017
      and kelpa2017flag=true and sap.activeflag is false  and r.currentschoolyear=2017 and gc.abbreviatedname in ('K','1','2')
      and not exists (select 1 from enrollment enrl where enrl.studentid=e.studentid and enrl.currentschoolyear=2018 and enrl.notes not like '%#$2018$#%');
...
set activeflag=true, modifieddate=now()
where kelpa2017flag=true and modifieddate='2017-07-31 12:48:27.467138+00' and activeflag=false 
and id in (select sap.id
 from enrollment  e 
inner join organizationtreedetail ot on ot.schoolid=e.attendanceschoolid
inner join studentassessmentprogram sap on sap.studentid=e.studentid
inner join assessmentprogram ap on ap.id=sap.assessmentprogramid
inner join gradecourse gc on e.currentgradelevel=gc.id
where ot.statedisplayidentifier in ('KS')  and ap.abbreviatedname='K-ELPA' and e.currentschoolyear in (2018)
and sap.activeflag is false and e.activeflag is true);  
          
-- Validation 

select e.currentschoolyear,gc.abbreviatedname,count( e.id)
 from enrollment  e 
inner join organizationtreedetail ot on ot.schoolid=e.attendanceschoolid
inner join studentassessmentprogram sap on sap.studentid=e.studentid
inner join assessmentprogram ap on ap.id=sap.assessmentprogramid
inner join gradecourse gc on e.currentgradelevel=gc.id
where ot.statedisplayidentifier='KS' and ap.abbreviatedname='K-ELPA' and e.currentschoolyear in (2017,2018)
and kelpa2017flag=true and sap.activeflag is true  --and case when e.currentschoolyear =2017 then  gc.abbreviatedname in ('K','1','2') else gc.abbreviatedname in ('K','1','2') end
and not exists (select 1 from enrollment enrl where enrl.studentid=e.studentid and enrl.currentschoolyear=2018 and enrl.notes not like '%#$2018$#%')
group by e.currentschoolyear,gc.abbreviatedname;
...
inner join gradecourse gc on e.currentgradelevel=gc.id
where ot.statedisplayidentifier='KS' and ap.abbreviatedname='K-ELPA' and e.currentschoolyear in (2017,2018)
      and kelpa2017flag=true and sap.activeflag is true  and r.currentschoolyear in (2017,2018) 
      and case when e.currentschoolyear =2017 then  gc.abbreviatedname in ('K','1','2') else gc.abbreviatedname in ('K','1','2') end
      and not exists (select 1 from enrollment enrl where enrl.studentid=e.studentid and enrl.currentschoolyear=2018 and enrl.notes not like '%#$2018$#%')
...
-- inner join roster rnew on split_part(rnew.notes,'#$2018$#',1)::text=r.id::text
where ot.statedisplayidentifier='KS' and ap.abbreviatedname='K-ELPA' and e.currentschoolyear in (2017,2018)
      and kelpa2017flag=true and sap.activeflag is false  and r.currentschoolyear in (2017,2018) 
     -- and case when e.currentschoolyear =2017 then  gc.abbreviatedname in ('K','1','2') else gc.abbreviatedname in ('K','1','2') end
      --and not exists (select 1 from enrollment enrl where enrl.studentid=e.studentid and enrl.currentschoolyear=2018 and enrl.notes not like '%#$2018$#%')
 group by e.currentschoolyear,r.currentschoolyear;           
