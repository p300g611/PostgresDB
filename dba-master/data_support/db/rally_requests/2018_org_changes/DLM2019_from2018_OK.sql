...
where ot.statedisplayidentifier in ('OK') and ap.abbreviatedname='DLM' and e.currentschoolyear=2018
...
inner join assessmentprogram ap on ap.id=sap.assessmentprogramid
inner join gradecourse gc on e.currentgradelevel=gc.id
where ot.statedisplayidentifier in ('OK')  and ap.abbreviatedname='DLM' and e.currentschoolyear in (2018,2019)
and sap.activeflag is true and case when e.currentschoolyear =2018 then  gc.abbreviatedname in ('3','4','5','6','7','8','10') else gc.abbreviatedname in ('3','4','5','6','7','8','10','11') end
and not exists (select 1 from enrollment enrl where enrl.studentid=e.studentid and enrl.currentschoolyear=2019 and enrl.notes not like '%#$2019$#%')
...
inner join gradecourse gc on e.currentgradelevel=gc.id
where ot.statedisplayidentifier in ('OK')  and ap.abbreviatedname='DLM' and e.currentschoolyear in (2018,2019)
group by gc.abbreviatedname,e.currentschoolyear
...
inner join gradecourse gc on e.currentgradelevel=gc.id
-- inner join enrollment  enew on split_part(enew.notes,'#$2018$#',1)::text=e.id::text
where ot.statedisplayidentifier in ('OK')  and ap.abbreviatedname='DLM' and e.currentschoolyear in (2017,2018)
...
inner join roster r on r.id=er.rosterid
inner join gradecourse gc on e.currentgradelevel=gc.id
where ot.statedisplayidentifier in ('OK')  and ap.abbreviatedname='DLM' and e.currentschoolyear in (2019,2018)
