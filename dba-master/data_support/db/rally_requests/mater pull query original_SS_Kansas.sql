SELECT DISTINCT
       ...
       FROM student
				   ...
				   LEFT JOIN studentstests ON (studentstests.studentid = student.id
							   ...
							   AND studentstests.contentareaname IN ('SS')
       ...
       WHERE student.state NOT IN ('flatland', 'DLM QC State', 'AMP QC State', 'KAP QC State', 'Playground QC State', 'Flatland',
							   ...
							   AND ( (studentstests.contentareaname IN ('SS') and studentstests.teststatus IN ('complete')) OR 
					                         (roster.statesubjectarea IN ('SS'))
					                        )
