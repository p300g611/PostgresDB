...
\COPY tmp_scenario5_raw FROM 'scenario5.csv' DELIMITER ',' CSV HEADER ;
...
BEGIN
      ...
      record_cnt:=1;
      err_msg := '';
FOR tmp_table IN   (SELECT row_num,error_msg,Student_First_Name,Student_Last_Name,State_Student_Identifier,upper(STATE) STATE,Hispanic_Ethinicity,Comprehensive_Race,ESOL,School_ID_Number,Current_School_Year,Date_of_Birth,Grade_Level FROM tmp_scenario5) 
   LOOP 
   record_cnt:=record_cnt+1;
   RAISE NOTICE 'adding row(%)>> Student_First_Name %: ,Student_Last_Name: %,State_Student_Identifier: %,STATE: % ,Hispanic_Ethinicity: %,Comprehensive_Race: %,ESOL: %,School_ID_Number: %,Current_School_Year: %,Grade_Level: %,Date_of_Birth: %' 
      ,record_cnt,tmp_table.Student_First_Name, tmp_table.Student_Last_Name, tmp_table.State_Student_Identifier,tmp_table.STATE,tmp_table.Hispanic_Ethinicity,tmp_table.Comprehensive_Race,tmp_table.ESOL,tmp_table.School_ID_Number,tmp_table.Current_School_Year,tmp_table.Grade_Level,tmp_table.Date_of_Birth; 
          BEGIN			  
          select updateStudentDemoGraphics(student_firstName :=tmp_table.Student_First_Name,
                                            ...
                                            birthDate:=cast(tmp_table.Date_of_Birth as date) ) INTO err_msg ;
...
\COPY (select * from tmp_scenario5 where error_msg ilike '%<error>%' ) to 'scenario5_error.csv' DELIMITER ',' CSV HEADER ;
drop table if exists  tmp_scenario5 ;
drop table if exists  tmp_scenario5_raw ;