...
\COPY tmp_scenario8_raw FROM 'scenario8.csv' DELIMITER ',' CSV HEADER ;   
  ...
  IF((select count(*) from student s 
   ...
   IF (tmp_table.State IN ('OK', 'MS') and tmp_table.Course_optional >'' ) THEN
                     ...
                     PERFORM createNewRosterWithSubAndCourse(state_student_identifiers := array[tmp_table.StudentState_ID]
                                                           , att_sch_displayidentifier := tmp_table.ReceivingSchool_Entry_Date
                                                           , ayp_sch_displayidentifier := tmp_table.ReceivingSchool_Entry_Date
                                                           , schoolyear := 2016
...
END;
$BODY$;
\COPY (select * from tmp_scenario8 where error_msg ilike '%<error>%' ) to 'scenario8_error.csv' DELIMITER ',' CSV HEADER ;
drop table if exists  tmp_scenario8 ;
drop table if exists  tmp_scenario8_raw ;