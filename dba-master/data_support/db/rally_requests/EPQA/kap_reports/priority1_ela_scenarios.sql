...
\copy (select * from tmp_epqa_scenario1) to 'tmp_epqa_scenario1_ela.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);
...
\copy (select * from tmp_epqa_scenario15) to 'tmp_epqa_scenario15_ela.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);
...
--and st.status=659 --IN PROGRESS TIMED OUT
--and (ot.districtid in (203, 394))
and gc.abbreviatedname in('3','4','5','6','7','8','10)
and not exists (select 1 from studentspecialcircumstance ssps where ssps.studenttestid=st.id)
group by st.studentid,st.enrollmentid,ot.schoolid,ot.districtid
having count(distinct sres1.taskvariantid)>5
--having count(distinct sres1.taskvariantid)=5
...
\copy (select * from tmp_epqa_scenario37) to 'tmp_epqa_scenario37_ela.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);
