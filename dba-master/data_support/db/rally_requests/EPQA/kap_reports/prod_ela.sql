...
--=======================================================================================================
--[ELA,KS,KAP,Report]: Grade 3
--=======================================================================================================

drop table if exists tmp_epqa_allscenarios_ela_g3;
with cte_stg2 as (
select
 st.studentid
...
,count(distinct sres2.taskvariantid) no_res_items_incl
from studentstests st 
inner join testsession ts on st.testsessionid=ts.id 
...
left join studentsresponses sres2 ON st.id = sres2.studentstestsid and sres2.taskvariantid=tstv.taskvariantid and sres2.score is not null and exld.tvid is null
...
,stg1.no_excluded_items                   "Stage1 #Excluded"
,stg1.no_res_items                        "Stage1 #Responded" 
,(stg1.no_req_items - stg1.no_excluded_items)  "Stage1 Required IncludedOnly"
,stg1.no_res_items_incl                   "Stage1 Responded IncludedOnly"
...
,stg2.no_excluded_items                   "Stage2 #Excluded"
,stg2.no_res_items                        "Stage2 #Responded" 
,(stg2.no_req_items - stg2.no_excluded_items)  "Stage2 #Required IncludedOnly"
,stg2.no_res_items_incl                   "Stage2 #Responded IncludedOnly"
,(stg1.no_excluded_items + stg2.no_excluded_items) "Total Excluded"
,((stg2.no_res_items - stg2.no_res_items_incl) + (stg2.no_res_items - stg2.no_res_items_incl)) "#Total Excluded Responded "
...
into temp tmp_epqa_allscenarios_ela_g3
from cte_stg1 stg1 
...
\copy (select * from tmp_epqa_allscenarios_ela_g3) to 'tmp_epqa_allscenarios_ela_grade3.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);
drop table if exists tmp_epqa_allscenarios_ela_g3;

--=======================================================================================================
--[ELA,KS,KAP,Report]: Grade 7
--=======================================================================================================

drop table if exists tmp_epqa_allscenarios_ela_g7;
with cte_stg2 as (
select
 st.studentid
,st.status as stg2_status
...
,sr.stage2hassccode                       "Stage 2 SC Flag"   
,sr.transferred                           "Transferred Flag"  
into temp tmp_epqa_allscenarios_ela_g7
from cte_stg1 stg1 
left join cte_stg2 stg2 on stg2.studentid=stg1.studentid 
inner join student s on stg1.studentid=s.id and s.activeflag is true
left join enrollment e on s.id=e.studentid  and e.activeflag is true 
...
\copy (select * from tmp_epqa_allscenarios_ela_g7) to 'tmp_epqa_allscenarios_ela_grade7.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);
drop table if exists tmp_epqa_allscenarios_ela_g7;
...
drop table if exists tmp_epqa_allscenarios_ela_g10;
with cte_stg2 as (
select
 st.studentid
,st.status as stg2_status
...
,sr.stage2hassccode                       "Stage 2 SC Flag"   
,sr.transferred                           "Transferred Flag"  
into temp tmp_epqa_allscenarios_ela_g10
from cte_stg1 stg1 
left join cte_stg2 stg2 on stg2.studentid=stg1.studentid 
inner join student s on stg1.studentid=s.id and s.activeflag is true
left join enrollment e on s.id=e.studentid  and e.activeflag is true 
...
\copy (select * from tmp_epqa_allscenarios_ela_g10) to 'tmp_epqa_allscenarios_ela_grade10.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);
drop table if exists tmp_epqa_allscenarios_ela_g10;
