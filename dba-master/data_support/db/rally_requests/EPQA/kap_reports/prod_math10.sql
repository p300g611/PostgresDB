...
drop table if exists tmp_epqa_allscenarios_math_g10;
with cte_stg2 as (
select
 st.studentid
...
,count(tv.id) no_req_items
,count(distinct sres1.taskvariantid) no_res_items
,count(distinct exld.tvid) no_excluded_items_responded
,st.status as stage2_status
from studentstests st 
inner join testsession ts on st.testsessionid=ts.id 
...
inner JOIN testsection tsec ON sts.testsectionid = tsec.id AND tsec.testid = st.testid
...
,stg1.no_excluded_items_responded         "Stage1  Excluded Items Responded"
,(stg1.no_res_items - stg1.no_excluded_items_responded) "Stage1 Included Items Responded"
,stg1.stage1_status    			 "Stage1 Status"	
...
,stg2.no_excluded_items_responded         "Stage2  Excluded Items Responded"
,(stg2.no_res_items - stg2.no_excluded_items_responded) "Stage2 Included Items Responded"
,stg2.stage2_status			  "Stage2 Status"
...
into temp tmp_epqa_allscenarios_math_g10
from cte_stg1 stg1 
...
\copy (select * from tmp_epqa_allscenarios_math_g10) to 'tmp_epqa_allscenarios_math_grade10.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);
drop table if exists tmp_epqa_exclude;
drop table if exists tmp_epqa_median;
drop table if exists tmp_sub_score;
drop table if exists tmp_sub_score;
