...
--=============================================================================================================================
--[ELA,KS,KAP,scenario1]:Transfer In or Out of district:In. Transfer before after Stage 1 or Stage 2:Before Stage 1
--===============================================================================================================================
drop table if exists tmp_ela_transfer_scenarios;
with cte_stg2 as (
select 
 st.studentid
...
into temp tmp_ela_transfer_scenarios
from studentstests st 
...
inner join stage stg on stg.id=tc.stageid and stg.activeflag is true
where ts.source='BATCHAUTO' and ts.operationaltestwindowid =10261
...
\copy (select * from tmp_ela_transfer_scenarios) to 'tmp_ela_transfer_scenarios1.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);
--=============================================================================================================================
--[ELA,KS,KAP,scenario5]:Transfer In or Out of district:Out. Transfer before after Stage 1 or Stage 2:Before Stage 1
--===============================================================================================================================
drop table if exists tmp_ela_transfer_scenarios;
with cte_stg2 as (
select 
...
--not exists (select 1 from studentstests stt where stt.enrollmentid =tmp.stg1_enrollmentid and stt.status in (84,86,659))
limit 1000;

\copy (select * from tmp_ela_transfer_scenarios) to 'tmp_ela_transfer_scenarios5.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);
...
--=============================================================================================================================
--[ELA,KS,KAP,scenario9]:Transfer In or Out of district:In. Transfer before after Stage 1 or Stage 2:After Stage 1
--===============================================================================================================================
drop table if exists tmp_ela_transfer_scenarios;
with tmp_dup as(
...
inner join testcollection tc on ts.testcollectionid=tc.id
where ts.source='BATCHAUTO' and ts.operationaltestwindowid =10261
and tc.contentareaid=3 and st.status in (85,86,659)
...
limit 1000;
\copy (select * from tmp_ela_transfer_scenarios) to 'tmp_ela_transfer_scenarios9.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);
--=============================================================================================================================
--[ELA,KS,KAP,scenario13]:Transfer In or Out of district:In. Transfer before after Stage 1 or Stage 2:After Stage 2
--===============================================================================================================================
drop table if exists tmp_ela_transfer_scenarios;
select distinct 
 s.statestudentidentifier                            "Student SSID"
...
and org.districtid=s.districtid and org.schoolid<>s.schoolid
limit 1000;
\copy (select * from tmp_ela_transfer_scenarios) to 'tmp_ela_transfer_scenarios13.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);

--=============================================================================================================================
--[ELA,KS,KAP,scenario17]:Transfer In or Out of district:Out(should receive generic report). Transfer before after Stage 1 or Stage 2:After Stage 1
--===============================================================================================================================
drop table if exists tmp_ela_transfer_scenarios;
with tmp_dup as(
select st.studentid,count(distinct ts.stageid) 
...
\copy (select * from tmp_ela_transfer_scenarios) to 'tmp_ela_transfer_scenarios17.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);

--=============================================================================================================================
--[ELA,KS,KAP,scenario21]:Transfer In or Out of district:Out. Transfer before after Stage 1 or Stage 2:After Stage 2
--===============================================================================================================================
drop table if exists tmp_ela_transfer_scenarios;
select distinct 
 s.statestudentidentifier                            "Student SSID"
...
\copy (select * from tmp_ela_transfer_scenarios) to 'tmp_ela_transfer_scenarios21.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);
