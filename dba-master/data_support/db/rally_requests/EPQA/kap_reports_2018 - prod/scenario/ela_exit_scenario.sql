...
where ts.operationaltestwindowid in (10261)  and ts.schoolyear=2018  and st.status in (477,86,85,659)
...
--=============================================================================================================================
--[ELA,KS,KAP, scenario1]:Stage 1:In Progress or In Progress Timed Out => 5 included answered. Stage 2: unused.
--===============================================================================================================================

drop table if exists tmp_ela_exit_scenarios;
with cte_stg1 as (
select 
 st.studentid
...
and ts.source='BATCHAUTO' and ts.operationaltestwindowid in (10261)
where (st.status =477 and ts.source='BATCHAUTO' and ts.operationaltestwindowid in (10261))--and st.startdatetime is null --this is required for unused) 
or st.id is null 
)
select distinct
...
into temp tmp_ela_exit_scenarios
from cte_stg1 stg1
...
\copy (select * from tmp_ela_exit_scenarios) to 'tmp_ela_exit_scenarios1.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);

--=============================================================================================================================
--[ELA,KS,KAP, scenario5]:Stage 1: Complete => included 5 answered. Stage: 2 unused
--===============================================================================================================================
...
left outer join testsession ts on st.testsessionid=ts.id and ts.schoolyear=2018 and ts.stageid=2
and ts.source='BATCHAUTO' and ts.operationaltestwindowid in (10261)
where (st.status =477 and st.startdatetime is null and ts.source='BATCHAUTO' and ts.operationaltestwindowid in (10261) ) or st.id is null
and not exists (select 1 from studentstests stt join testsession tts on stt.testsessionid=tts.id and tts.schoolyear=2018 and tts.stageid=2
                 where stg1.studentid=stt.studentid  and tts.source='BATCHAUTO' and tts.operationaltestwindowid in (10261) and stt.status in (86,85)) 
)
select distinct
s.statestudentidentifier                 "Student SSID"
,s.id                                     studentid
...
\copy (select * from tmp_ela_exit_scenarios) to 'tmp_ela_exit_scenarios5.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);


--=============================================================================================================================
--[ELA,KS,KAP, scenario9]:Stage 2: In Progress or In Progress Timed Out => included 5 answered.
--===============================================================================================================================
...
\copy (select * from tmp_ela_exit_scenarios) to 'tmp_ela_exit_scenarios9.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);



--=============================================================================================================================
--[ELA,KS,KAP, scenario13]:Stage 2: Complete => 5 included answered.
--===============================================================================================================================
...
\copy (select * from tmp_ela_exit_scenarios) to 'tmp_ela_exit_scenarios13.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);
