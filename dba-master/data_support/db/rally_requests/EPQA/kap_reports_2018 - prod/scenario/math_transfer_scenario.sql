...
--=============================================================================================================================
--[MATH,KS,KAP,scenario2]:Transfer In or Out of district:In. Transfer before after Stage 1 or Stage 2:Before Stage 1
--===============================================================================================================================
drop table if exists tmp_math_transfer_scenarios;
with cte_stg2 as (
select 
 st.studentid
...
into temp tmp_math_transfer_scenarios
from studentstests st 
...
\copy (select * from tmp_math_transfer_scenarios) to 'tmp_math_transfer_scenarios2.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);
--=============================================================================================================================
--[MATH,KS,KAP,scenario6]:Transfer In or Out of district:Out. Transfer before after Stage 1 or Stage 2:Before Stage 1
--===============================================================================================================================
drop table if exists tmp_math_transfer_scenarios;
with cte_stg2 as (
select 
...
\copy (select * from tmp_math_transfer_scenarios) to 'tmp_math_transfer_scenarios6.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);
...
inner join testtype tt on ets.testtypeid=tt.id and tt.activeflag is true  
where e.currentschoolyear=2018 and sap.assessmentprogramid=12 and sub.id=1 and tt.id=2 
and exists (select 1 from studentstests stt
where e.studentid=stt.studentid and transferedenrollmentid<>enrollmentid and transferedenrollmentid is not null);
--=============================================================================================================================
--[MATH,KS,KAP,scenario10]:Transfer In or Out of district:In. Transfer before after Stage 1 or Stage 2:After Stage 1
--===============================================================================================================================
drop table if exists tmp_math_transfer_scenarios;
with tmp_dup as(
...
inner join testcollection tc on ts.testcollectionid=tc.id
where ts.source='BATCHAUTO' and ts.operationaltestwindowid =10261
and tc.contentareaid=440 and st.status in (85,86,659)
...
limit 1000;
\copy (select * from tmp_math_transfer_scenarios) to 'tmp_math_transfer_scenarios10.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);
--=============================================================================================================================
--[MATH,KS,KAP,scenario14]:Transfer In or Out of district:In. Transfer before after Stage 1 or Stage 2:After Stage 2
--===============================================================================================================================
drop table if exists tmp_math_transfer_scenarios;
select distinct
 s.statestudentidentifier                            "Student SSID"
...
and org.districtid=s.districtid and org.schoolid<>s.schoolid
limit 1000;
\copy (select * from tmp_math_transfer_scenarios) to 'tmp_math_transfer_scenarios14.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);

--=============================================================================================================================
--[MATH,KS,KAP,scenario18]:Transfer In or Out of district:Out(should receive generic report). Transfer before after Stage 1 or Stage 2:After Stage 1
--===============================================================================================================================
drop table if exists tmp_math_transfer_scenarios;
with tmp_dup as(
select st.studentid,count(distinct ts.stageid) 
...
\copy (select * from tmp_math_transfer_scenarios) to 'tmp_math_transfer_scenarios18.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);

--=============================================================================================================================
--[MATH,KS,KAP,scenario22]:Transfer In or Out of district:Out. Transfer before after Stage 1 or Stage 2:After Stage 2
--===============================================================================================================================
drop table if exists tmp_math_transfer_scenarios;
select distinct 
 s.statestudentidentifier                            "Student SSID"
...
\copy (select * from tmp_math_transfer_scenarios) to 'tmp_math_transfer_scenarios22.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);