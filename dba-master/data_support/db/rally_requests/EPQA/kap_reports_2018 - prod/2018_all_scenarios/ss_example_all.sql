...
-- insert into tmp_epqa_exclude
 ...
 -- where externalid in (
 -- select distinct taskvariantid from excludeditems where schoolyear = 2018 and assessmentprogramid = 12 and subjectid = 443);
...
where rpm.assessmentprogramid =12 and rpm.contentareaid=443  and rpm.schoolyear=2018  and rpm.subscoredefinitionname  is null
 and drpm.assessmentprogramid =12 and drpm.contentareaid=443 and drpm.schoolyear=2018 and drpm.subscoredefinitionname is null 
 and srpm.assessmentprogramid =12 and srpm.contentareaid=443 and srpm.schoolyear=2018 and srpm.subscoredefinitionname is null
order by org.schoolid, rpm.gradeid;


-- view only lines:claim names different for ela
--step3: we need to exclude the items by the subject (no ss subscoredefinitionname in the table subscoresdescription)
...
where sr.schoolyear=2018 and sr.assessmentprogramid=12 and sr.contentareaid=443
...
where st.activeflag is true and ts.operationaltestwindowid =10258  and ts.schoolyear=2018  and st.status in (86,85,659)
and tc.contentareaid=443 and ts.source='BATCHAUTO'   and stg.code in ('Stg1','Stg2') 
...
and tc.contentareaid=443 and stg.code='Stg2' 
...
,rss.Claim_1_all                          "PHYSICAL AND CHEMICAL SCIENCES"
,rss.Claim_2_all                          "LIFE SCIENCES"
,rss.Claim_3_all                          "EARTH AND SPACE SCIENCES"
...
\copy (select * from tmp_epqa_allscenarios) to 'tmp_epqa_allscenarios_sci_grade5.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);
