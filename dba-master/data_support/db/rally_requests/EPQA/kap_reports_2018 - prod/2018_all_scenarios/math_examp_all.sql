...
-- insert into tmp_epqa_exclude
 ...
 -- where externalid in (
 -- select distinct taskvariantid from excludeditems where schoolyear = 2018 and assessmentprogramid = 12 and subjectid = 440);


-- step2:we need to find the school,district,state score and standarderror
drop table if exists tmp_epqa_median;
select distinct org.schoolid,rpm.gradeid
...
where rpm.assessmentprogramid =12 and rpm.contentareaid=440  and rpm.schoolyear=2018  and rpm.subscoredefinitionname  is null
 and drpm.assessmentprogramid =12 and drpm.contentareaid=440 and drpm.schoolyear=2018 and drpm.subscoredefinitionname is null 
 and srpm.assessmentprogramid =12 and srpm.contentareaid=440 and srpm.schoolyear=2018 and srpm.subscoredefinitionname is null
...
where sr.schoolyear=2018 and sr.assessmentprogramid=12 and sr.contentareaid=440
...
and ts.operationaltestwindowid =10261 and tc.contentareaid=440 and stg.code='Stg2' 
and st.status in (84, 85, 86, 659) --UNUSED, IN PROGRESS, COMPLETED,  IN PROGRESS TIMED OUT
and gc.abbreviatedname = '3'
-- and st.studentid = (select id from student where stateid = 51 and statestudentidentifier = '5432263343')
)
,cte_stg1 as (
select 
 st.studentid
...
\copy (select * from tmp_epqa_allscenarios_math) to 'tmp_epqa_allscenarios_math_grade3.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);
