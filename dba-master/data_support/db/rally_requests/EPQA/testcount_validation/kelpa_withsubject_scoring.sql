...
,count (distinct case when stg.code='Spkng' and ap.id=47 and ccq.studentstestsid is not null and ccq.status=630 then st.studentid else null end ) stage_Spkng_scored_pending
...
,count (distinct case when stg.code='Wrtng' and ap.id=47 and ccq.studentstestsid is not null and ccq.status=630 then st.studentid else null end)  stage_Wrtng_scored_pending
...
left outer join (select sc.testsessionid,scs.studentstestsid,ccq.status
...
left outer join ccqscore ccq on ccq.scoringassignmentstudentid=scs.id and sccq.id=ccq.scoringassignmentscorerid and ccq.status in (629,630,631)
where sc.rosterid is not null  
group by sc.testsessionid,scs.studentstestsid,ccq.status) ccq on ccq.testsessionid=ts.id and ccq.studentstestsid=st.id
...
,noof_unusedtests          noof_unusedtests
,noof_inprogresstests      noof_inprogresstests
,noof_completedtests       noof_completedtests
,noof_testscreated         noof_testscreated
,stage_Lstng               stage_Lstng   
,stage_Rdng                stage_Rdng   
,stage_Spkng               stage_Spkng   
,stage_Spkng_score_assigned         stage_Spkng_score_assigned
,stage_Spkng_score_inprogress       stage_Spkng_score_inprogress
,stage_Spkng_scored_pending         stage_Spkng_scored_pending
,stage_Spkng_scored_completed       stage_Spkng_scored_completed
,stage_Wrtng                        stage_Wrtng 
,stage_Wrtng_score_assigned         stage_Wrtng_score_assigned
,stage_Wrtng_score_inprogress       stage_Wrtng_score_inprogress
,stage_Wrtng_scored_pending         stage_Wrtng_scored_pending
,stage_Wrtng_scored_completed       stage_Wrtng_scored_completed   
,stage_all                          stage_all
,stage_all_score
into temp tmp_epqa
from cte_std tmp
...
\copy (select * from tmp_epqa) to 'kelpa_withsubject_scoring.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);