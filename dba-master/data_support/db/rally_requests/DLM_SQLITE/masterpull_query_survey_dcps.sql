CREATE TABLE IF NOT EXISTS masterpull_dcps as
SELECT DISTINCT
       student.id,
       ...
       WHERE  taskvariant.contentframeworkdetailcode  in ('DCPS.01') and 
	   student.state NOT IN ('flatland', 'DLM QC State', 'AMP QC State', 'KAP QC State', 'Playground QC State', 'Flatland',
			   ...
			   AND ( (studentstests.teststatus is null) OR
				 (studentstests.teststatus = 'unused' AND studentstests.createddate > '2017-08-01') OR
				 (studentstests.teststatus IN ('unused','inprogress','complete')
					   AND (studentstests.startdatetime BETWEEN '2017-08-01' AND '2018-07-01 23:59:59.999'
						)
				  )
				);