SELECT DISTINCT
       ...
       student.final_ela,
       student.final_math,
       student.comm_band as final_sci,
       test.testname,
       ...
       FROM student
				   ...
				   LEFT JOIN studentstests ON (studentstests.studentid = student.id
							   ...
							   AND studentstests.contentareaname IN ('Sci','SS')
       ...
       WHERE student.state NOT IN ('flatland', 'DLM QC State', 'AMP QC State', 'KAP QC State', 'Playground QC State', 'Flatland',
							   ...
							   AND ( 
					                         (roster.statesubjectarea IN ('Sci')) OR
					                         (roster.statesubjectarea IN ('SS') AND student.state ='Wisconsin' AND taskvariant.externalid in (58644,58645,58095)) OR
					                         (studentstests.teststatus IN ('complete') and studentstests.contentareaname IN ('Sci')) OR
					                         (studentstests.teststatus IN ('complete') and studentstests.contentareaname IN ('SS')
					                             AND student.state ='Wisconsin' AND taskvariant.externalid in (58644,58645,58095)) 
