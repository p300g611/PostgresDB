...
where st.activeflag is true 
 and e.currentschoolyear=2017 AND tp.programname ='Summative'  and ap.id=12 and tc.contentareaid=440;
...
where str.schoolyear=2017 and str.assessmentprogramid=12 and str.contentareaid=440
...
--\copy (select * from tmp_final_extract) to 'kap_extract_ela.txt' (FORMAT CSV,DELIMITER('|'), HEADER TRUE, FORCE_QUOTE *);
