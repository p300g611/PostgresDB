...
\COPY tmp_possibleval FROM '/srv/extracts/CETE/tmp_possibleval.csv' DELIMITER ',' CSV HEADER ;  
...
FOR tmp_table IN (select distinct taskvariantid from tmp_possibleval order by 1)
  ...
  --raise info 'id:%',tmp_table.taskvariantid;
                     FOR tmp_table IN (select taskvariantid, scores from tmp_possibleval
			               where taskvariantid=tmp_table.taskvariantid and scores::numeric>0 order by 1,2 desc)
			  LOOP 
...
\copy (select distinct * from tmp_final_report) to '/srv/extracts/CETE/tmp_final_report.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *); 