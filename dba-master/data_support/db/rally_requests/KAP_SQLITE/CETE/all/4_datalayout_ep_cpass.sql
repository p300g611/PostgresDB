...
where st.activeflag is true 
 and e.currentschoolyear=2017 AND tp.programname ='Summative'  and ap.id=11;
...
where str.schoolyear=2017 and str.assessmentprogramid=11
...
WHERE s.stateid in (select organizationid from orgassessmentprogram where assessmentprogramid=11) AND e.currentschoolyear=2017
...
where ap.id=11  and ts.schoolyear=2017 and ccqi.nonscoringreason is not null
...
\copy (select * from tmp_final_extract) to 'cpass_extract_all.txt' (FORMAT CSV,DELIMITER('|'), HEADER TRUE, FORCE_QUOTE *);
