...
where st.activeflag is true 
 and e.currentschoolyear=:v_year AND tp.programname ='Summative'  and ap.id=:v_ap and tc.contentareaid=:v_sub and gc.abbreviatedname=:'v_grade';
...
where str.schoolyear=:v_year and str.assessmentprogramid=:v_ap and str.contentareaid=:v_sub
...
WHERE s.stateid in (select organizationid from orgassessmentprogram where assessmentprogramid=:v_ap) AND e.currentschoolyear=:v_year
...
where ap.id=:v_ap  and ts.schoolyear=:v_year and ccqi.nonscoringreason is not null
...
\copy (select * from tmp_final_extract) to 'kap_extract_ela_grade.txt' (FORMAT CSV,DELIMITER('|'), HEADER TRUE, FORCE_QUOTE *);
