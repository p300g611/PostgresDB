...
INNER JOIN gradecourse gcc on gcc.id=e.currentgradelevel
...
where st.activeflag is true 
 and e.currentschoolyear=2017 AND tp.programname ='Summative'  and ap.id=12 and tc.contentareaid=3 and gcc.abbreviatedname='1';
...
where str.schoolyear=2017 and str.assessmentprogramid=12 and str.contentareaid=3
...
--\copy (select * from tmp_final_extract) to 'kap_extract_ela_grade1.csv' (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);       
--\copy (select * from tmp_final_extract) to 'kap_extract_ela.txt'  delimiter '|' CSV HEADER ;
\copy (select * from tmp_final_extract) to 'kap_extract_ela_grade1.txt' (FORMAT CSV,DELIMITER('|'), HEADER TRUE, FORCE_QUOTE *);
