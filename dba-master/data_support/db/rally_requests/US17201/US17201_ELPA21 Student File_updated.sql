   SELECT distinct on(e.studentid)
		...
		,''																					 "Teacher ID "
		,'spring'																			 "Term"
		,'English Language Proficiency'														 "Credit"
		...
		,(select a.surname  from aartuser a
			...
			where uo.organizationid=s.stateid limit 1)											"Teacher Last Name"
		,''																						"Teacher Middle Initial"
		,(select a.firstname  from aartuser a
			inner join usersorganizations uo on uo.aartuserid=a.id 
			inner join userorganizationsgroups ug on uo.id=ug.userorganizationid
			inner join groups g on g.id = ug.groupid and g.groupname ='District Test Coordinator'
			where uo.organizationid=s.stateid limit 1) 											 "Teacher First Name"
		...
		FROM student s
						...
						INNER JOIN organization o ON o.id=e.aypschoolid
