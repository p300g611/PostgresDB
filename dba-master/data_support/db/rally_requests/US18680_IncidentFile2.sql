...
-- Find the student and codes related to window2(EOI)
  select distinct statestudentidentifier,
		    ...
		    where stb.operationalwindowid in (10142);
...
\copy (SELECT distinct statestudentidentifier,contentframeworkdetailcode,issue_code,state FROM tmp_Incidentfile_ee order by 1) TO 'tmp_Incidentfile_ee.csv' DELIMITER ',' CSV HEADER;--(FORMAT CSV, HEADER TRUE, FORCE_QUOTE *);
\copy (SELECT distinct statestudentidentifier,grf contentframeworkdetailcode,issue_code,state FROM tmp_Incidentfile_ee order by 1) TO 'tmp_Incidentfile_grf.csv' DELIMITER ',' CSV HEADER;

--Issue code2:Misrouting due to NA scores not being scored as incorrect.
