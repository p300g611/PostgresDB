...
-- select sl.labelnumber,globalpagenum,slp.*,optional,sr.responselabel,sr.responsevalue from surveylabel  sl 
-- left outer join surveylabelprerequisite slp on slp.surveylabelid=sl.id 
...
--select tgt.surveyid ,sp.globalpagenum,sp.iscompleted,sp.id pageid
 select count(distinct tgt.surveyid)
 from page678 tgt
 join page9_nq19 src on tgt.surveyid=src.surveyid 
 inner join surveypagestatus sp on sp.surveyid=tgt.surveyid and sp.globalpagenum=9 and iscompleted is false
...
-- │ count │ 13 │
 ...
 --***************************************--
--  UPDATE 1
--only below neet to run for this validation3_c
begin;
update surveypagestatus
...
--and sp.globalpagenum in (9,6,7,8) 
 and sp.globalpagenum in (9)  --and sp.iscompleted is true
) and iscompleted is true and globalpagenum=9;
commit;
--**********************************************--