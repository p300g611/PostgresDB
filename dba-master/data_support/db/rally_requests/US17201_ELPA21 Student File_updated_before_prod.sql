   SELECT distinct on(e.studentid)
                                ...
                                ,cast(regexp_replace(o.organizationname, '[^-a-zA-Z0-9.\,()&'' ]', '', 'g')  as character varying(75))                                              "School Name"
                                ,''                                                                                              "Local ID"
                                ,row_number () over(order by s.id )                                                              "Teacher ID "
                                ,'spring'                                                                                        "Term"
                                ,''                                                                                              "Credit"
                                ...
                                ,cast(regexp_replace(s.legalfirstname, '[^-a-zA-Z.'' ]', '', 'g') as character varying(35))        "Student First Name"
                                ,cast(s.legalmiddlename as varchar(1))                                                             "Student Middle Initial"
                                ,case when to_char(s.dateofbirth::date, 'MMDDYYYY')  >''  then  to_char(s.dateofbirth::date, 'MMDDYYYY')
                                         else               to_char('1900/01/01'::date, 'MMDDYYYY') end                                   "Date of Birth"
				...
				,'N'                                                   "Economic Disadvantaged"
                                ,case when (select a.id from usersorganizations uo  
                                                    ...
                                                    inner join groups g on g.id = ug.groupid and g.groupcode ='PRN'
                                                    where uo.organizationid=o.id  limit 1) is null then 
                                  (select a.surname from  usersorganizations uo 
                                                    INNER JOIN aartuser a on a.id=uo.aartuserid
                                                    inner join userorganizationsgroups ug on uo.id=ug.userorganizationid
                                                    inner join groups g on g.id = ug.groupid and g.groupcode ='DTC'
